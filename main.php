<?php
/**
* Telegram Bot example for dovedormireinsardegnabot
 ...
 function shell($telegram,$text,$chat_id,$user_id,$location,$reply_to_msg)
{
	date_default_timezone_set('Europe/Rome');
	$today = date("Y-m-d H:i:s");
	if (strpos($text,'@dovedormireinsardegnabot') !== false) $text=str_replace("@dovedormireinsardegnabot ","",$text);

	if ($text == "/start" || $text == "©️ Informazioni") {
		$img = curl_file_create('logosardegna.png','image/png');
		$contentp = array('chat_id' => $chat_id, 'photo' => $img);
		$telegram->sendPhoto($contentp);
		$reply = "Benvenuto. Per ricercare una struttura ricettiva della Regione Sardegna, censita dal Dipartimento regionale del turismo dello sport e dello spettacolo, digita il nome del Comune oppure clicca sulla graffetta (📎) e poi 'posizione' . Puoi anche ricercare per parola chiave nel titolo anteponendo il carattere ?. Verrà interrogato il DataBase openData utilizzabile con licenza CC0 presente su http://dati.regione.sardegna.it/dataset/punti-di-interesse-strutture-turistiche. In qualsiasi momento scrivendo /start ti ripeterò questo messaggio di benvenuto.\nQuesto bot è stato realizzato da @piersoft con il prezioso contributo di @matteoenna e potete migliorare il codice sorgente con licenza MIT che trovate su https://github.com/piersoft/dovedormireinsardegnabot.\nLa propria posizione viene ricercata grazie al geocoder di openStreetMap con Lic. odbl.\n";
...
if(strpos($text,'!') !== false) {
	...
	//			sleep (1);
		...
		//		sleep (1);
			...
			$homepage="";
$json_string=file_get_contents("/usr/www/piersoft/dovedormireinsardegnabot/db/ricettive.json");
...
//	if (strpos($filterint,$textint) !== false ){
	...
	if (strip_tags($parsed_json->{'features'}[$i]->{'properties'}->{'nome_comune'}) !=null)	$homepage .= "Località: <b>".$parsed_json->{'features'}[$i]->{'properties'}->{'nome_comune'}."</b> (".$parsed_json->{'features'}[$i]->{'properties'}->{'prov'}.")\n";
